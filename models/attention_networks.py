...
def gumble_transform(confident_score, t, isTrain=True):
    # t is the temperature --> scalar
    if isTrain:
        noise = torch.rand(confident_score.size(), requires_grad=False).to('cuda')
        noise = (noise + 0.00001) / 1.001
        noise = - torch.log(- torch.log(noise))

        confident_score = (confident_score + 0.00001) / 1.001
        confident_score = (torch.log(confident_score) + noise) / t
    else:
        confident_score = torch.log(confident_score) / t

    confident_score = F.softmax(confident_score, dim=1)

    return confident_score

def gumble_transform_sigmoid(confident_score, t, isTrain=True):
    # t is the temperature --> scalar
    if isTrain:
        noise = torch.rand(confident_score.size(), requires_grad=False).to('cuda')
        noise = (noise + 0.00001) / 1.001
...
class KL_loss(nn.Module):
    ...
    def forward(self, x):
        g = x.mean()
        y = self.rho*torch.log(self.rho/g) + (1-self.rho)*torch.log((1-self.rho)/(1-g))
        return y
        #Rho = x*0+self.rho 
        #y = Rho*torch.log(Rho/x) + (1-Rho)*torch.log((1-Rho)/(1-x))
        #return y.mean(), y
...
class _DecoderUpBlock_Upsampling(nn.Module):
    def __init__(self, input_nc, middle_nc, output_nc, norm_layer=nn.BatchNorm2d, nonlinearity=nn.PReLU(), use_bias=False):
...
class _Attention_FullRes(nn.Module):
    ...
    def forward(self, input):
        # input a list of kernel_size and strides
...
class _Attention_Depth_FullRes(nn.Module):
    def __init__(self, input_nc, output_nc, ngf=64, n_blocks=6, norm='batch', activation='PReLU', drop_rate=0, add_noise=False, gpu_ids=[]):
        super(_Attention_Depth_FullRes, self).__init__()
        ...
        attention_proj = [
            nn.Conv2d(ngf, 2, kernel_size=1, padding=0)
        ]
        ...
        self.encoder = nn.Sequential(*encoder)
        self.decoder = nn.Sequential(*decoder)
        self.attention_proj = nn.Sequential(*attention_proj)
        self.depth_proj = nn.Sequential(*depth_proj)

    def forward(self, input, tau, isTrain):
        # input a list of kernel_size and strides
        feature = self.encoder(input)
        result = [feature]
        # print(feature.size())
        decoder = self.decoder(feature)
        result.append(decoder)
        attention = self.attention_proj(decoder)
        attention = F.softmax(attention, dim=1)
        attention_gumble = gumble_transform(attention, tau, isTrain)
        attention_gumble = attention_gumble[:, 0, :, :].unsqueeze(1)
        output = decoder * attention_gumble
        result.append(attention_gumble)
        depth = self.depth_proj(output)
        result.append(depth)
        return result

class _Attention_Depth_FullRes_Sigmoid(nn.Module):
    def __init__(self, input_nc, output_nc, ngf=64, n_blocks=6, norm='batch', activation='PReLU', drop_rate=0, add_noise=False, gpu_ids=[]):
        super(_Attention_Depth_FullRes_Sigmoid, self).__init__()
        ...
        attention_proj = [
            nn.Conv2d(ngf, 1, kernel_size=1, padding=0)
        ]
    ...
    def forward(self, input, tau, isTrain):
        ...
        attention_gumble = gumble_transform_sigmoid(attention, tau, isTrain)
        # attention_gumble = attention_gumble[:, 0, :, :].unsqueeze(1)
        output = decoder * attention_gumble
        result.append(attention_gumble)
        # print(attention_gumble)
        depth = self.depth_proj(output)
        result.append(depth)
        return result

class _Attention_Pool(nn.Module):
    def __init__(self, input_nc, output_nc, output_size, ngf=64, n_blocks=6, norm='batch', activation='PReLU', drop_rate=0, add_noise=False, gpu_ids=[]):
        super(_Attention_Pool, self).__init__()
    ...
    def forward(self, input):
        ...
        # for i in len(k_s_list):
        #     kernel_size, stride = k_s_list[i]
        #     output_pool_i = F.max_pool2d(output, kernel_size=kernel_size, stride=stride)
        ...
        output_pool = F.avg_pool2d(output, kernel_size=(int(H/self.output_h), int(W/self.output_w)),
            stride=(int(H/self.output_h), int(W/self.output_w)))
...
class _SimplifiedUNetGenerator(nn.Module):
    def __init__(self, input_nc, output_nc, ngf=64, layers=4, norm='batch', activation='PReLU', drop_rate=0, add_noise=False, gpu_ids=[],
                 weight=0.1):
