...
class finetune_the_whole_system_with_depth_loss(base_model):
	def __init__(self, args, dataloaders_xLabels_joint, dataloaders_single):
		super(finetune_the_whole_system_with_depth_loss, self).__init__(args)
		...
		if self.isTrain:
			...
			# load the "best" depth predictor D (from step 5)
	...
	def _get_project_name(self):
		return 'finetune_the_whole_system_with_depth_loss'
	...
	def train(self):
		...
		since = time.time()
		best_loss = float('inf')
		set_requires_grad(self.attModule, requires_grad=False) # freeze attention module A
		...
		for epoch in range(self.total_epoch_num):
			...
			for sample_dict in self.dataloaders_xLabels_joint:
				...
				with torch.set_grad_enabled(phase=='train'):
					...
					total_loss = 0.
					inpainted_depth_loss = self.compute_depth_loss(reconst_img, depthListReal, self.depthEstModel, valid_mask)
					# add translated image to finetune the whole system might gives better results (normally could also be commented out)
					translated_depth_loss = self.compute_depth_loss(r2s_img, depthListReal, self.depthEstModel, valid_mask)
					syn_depth_loss = self.compute_depth_loss(imageListSyn, depthListSyn, self.depthEstModel)
					total_loss += (inpainted_depth_loss + translated_depth_loss + syn_depth_loss)
					if self.use_apex:
				...
				if self.use_tensorboardX:
					...
					# add loss values
					loss_val_list = [total_loss, inpainted_depth_loss, translated_depth_loss, syn_depth_loss]
					loss_name_list = ['total_loss', 'inpainted_depth_loss', 'translated_depth_loss', 'syn_depth_loss']
				...
				if iterCount % 20 == 0:
					loss_summary = '\t{}/{}, total_loss: {:.7f}, inpainted_depth_loss: {:.7f}, translated_depth_loss: {:.7f}, syn_depth_loss: {:.7f}'.format(
						iterCount, len(self.dataloaders_xLabels_joint), total_loss, inpainted_depth_loss, translated_depth_loss, syn_depth_loss)
